...
class RobotConfig:
    ...
    arm_joint_limits_lower: np.ndarray = field(
        default_factory=lambda: np.array([-5.89, -3.76, -2.89, -3.07, -2.89, -0.01, -2.89]),
        init=False
    )
    arm_joint_limits_upper: np.ndarray = field(
        default_factory=lambda: np.array([5.89, 3.76, 2.89, -0.06, 2.89, 3.75, 2.89]),
        init=False
    )
...
class ConfigPresets:
    ...
    def advanced_smooth():
        ...
        robot.planner = PlannerConfig()
        robot.planner.version = PlannerVersion.SMOOTH
        ...
        scenario.playground_type = PlaygroundType.MEDIUM
        scenario.render = True
